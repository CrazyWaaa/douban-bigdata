...
const allLinks = computed(() => [...links1.value, ...links2.value])

const sankeyOption = computed(() => {
  const payload = { nodes: nodes.value, links: allLinks.value }
  return buildSankeyOption(payload)
})
...
async function load() {
  ...
  try {
    ...
    const genres   = (Array.isArray(genreList) ? genreList : []).slice(0, 5)
    const countries = (Array.isArray(countryList) ? countryList : []).slice(0, 5)
    const years    = (Array.isArray(yearList) ? yearList : []).slice(0, 12)
    ...
    for (const c of countries) for (const d of decadeArr) {
      const v = Math.max(1, Math.round(Number(c.count || 0) * 0.2 / decadeArr.length))
