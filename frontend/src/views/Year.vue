<template>
  <div class="dim-page">
    <header class="page-head fade-up">
      <div>
        <h1 class="page-title">年代分布</h1>
        <p class="page-sub">按上映年代聚合，看年代偏好与年代均分</p>
      </div>
    </header>
      ...
      <UiChartCard title="Top10 数量" class="fade-up dim-grid__full" style="animation-delay: 180ms">
        <div class="dim-list">
          <div v-for="(it, i) in items.slice(0, 10)" :key="it.name" class="dim-list__row" :style="rowStyle(i)">
            ...
            <span class="dim-list__name">{{ it.name }} 年</span>
...
const PIE_COLORS = ["#22d3ee", "#34d399", "#a78bfa", "#f472b6", "#f59e0b", "#38bdf8", "#fb923c", "#fb7185", "#60a5fa", "#facc15"];

onMounted(async () => {
  try {
    const data = await api.byAvg(DIM, 30);
    items.value = data?.data || [];
    console.log("[Year] dim=", DIM, "items.length=", items.value.length, "first=", items.value[0]);
  } catch (e) {
...
const dualOption = computed(() => ({
  ...
  series: [
    { name: "数量", type: "bar", data: items.value.slice(0, 10).map(d => d.count),
      itemStyle: { color: { type: "linear", x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: "#22d3ee" }, { offset: 1, color: "rgba(34,211,238,.15)" }] }, borderRadius: [4,4,0,0] } },
...
const pieOption = computed(() => {
  ...
  return {
    tooltip: { trigger: "item", formatter: "{b} 年<br/>数量: <b>{c}</b> 部 ({d}%)" },
